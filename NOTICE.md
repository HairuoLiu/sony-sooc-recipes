...
### 相机模拟批次（v0.3.0 的 18 款）— 只借「事实」，不借「表达」

v0.3.0 新增的 18 款「相机调色模拟」配方（Pentax 11 款、哈苏 HNCS 3 款、徕卡机身
2 款、GR 电影调 2 款）由本项目**自写**，参数依据是各厂商对自己机身色彩模式的
**公开描述**——例如宾得官网逐条说明每个 Custom Image 的观感（「低调高反差色彩收敛」
「高调低反差饱和收敛」等）。机身模式的观感特征是事实，不是任何人的创作表达。

- **触发背景**：用户提供了一批「相机模拟 LUT」合集作参考。其中哈苏/徕卡 M9/M240/
  宾得/理光一套的许可为 **署名-非商业性使用-禁止演绎（BY-NC-ND）**，把 LUT 数值
  转换或转码后发布即构成衍生作品，**本仓库不做、也没做**。
- **本项目明确没有做什么**：
  - **未转录、转码或拟合任何第三方 LUT 的数值**（一个都没有）
  - **未分发任何 LUT 文件**
  - Dehancer 胶片合集（68 款，商业插件 Dehancer 的专有 profile）与出处不明的
    Leica 缩写名合集**未收录**
- **因此这些条目是本仓库的原创近似**：`source: authored-here`、`verified: false`，
  命名用于描述其试图接近的机身观感，与 Liit 条目的处理原则一致。
...
|---|---|---|---|
| 本仓库代码与文档 | 原创 | MIT | ✔ |
| 102 款可编译配方参数 | voxivoid（77）+ 本项目（25） | MIT | ✔ |
...
| 相机模拟 LUT 合集（用户提供） | 私人收藏 | BY-NC-ND 等 | ✘ 仅「哪些机身模式存在」这一事实，见上文 |
