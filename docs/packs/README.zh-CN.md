...
本文件夹记录 **当前代码树附带的每一个 APK**：全量版（all-in-one）加十个单品牌包。
...
|---|---|---:|---|---|
| **Sony SOOC Recipes**（全量版） | `com.hairuoliu.sonysoocrecipes` | **149** | 15 | [all-in-one](all-in-one.zh-CN.md) |
...
| `cinema` | Cinema LUT | `com.hairuoliu.sonysoocrecipes.cinema` | **9** | 1 | [cinema](cinema.zh-CN.md) |

**149 + 20 + 16 + 10 + 11 + 20 + 11 + 26 + 4 + 8 + 9 = 284** 条「配方列表」，但底层目录其实只有
**164** 个滤镜条目（149 已编译 + 15 仅供查阅）。总和大于 164，是因为品牌包只是共享目录上的
*视图*，而非副本——全量版里的 149 条配方，与各个单品牌包里的配方是同一批，只是汇在一处还是
拆成十处呈现的区别。
