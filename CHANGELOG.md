...
structural review: three gaps that a test could close were closed, and one dead file was
removed.

- **`tools/preview_picker.py` and `assets/ui/PickerView.java` no longer drift apart silently.**
  Both files state the browser's geometry — padding, the header and footer reserves, the row
  height and the 9/13/10 sp text sizes — as six separate numbers, and the preview is the only
  way to judge those pixels without a camera. `TestPreviewMirrorsTheBrowser` now extracts each
  value from both sides and compares them, so changing one without the other fails with both
  numbers in the message. Every visual defect fixed since v0.7.2 was found by eye against this
  preview, which is exactly why the two copies had to be tied together.
- **The third copy of the upstream pin is now guarded.** The same upstream SHA is written in
  `catalog/filters.json`, `.github/workflows/release.yml` and `tools/build_apk.sh`, but
  `TestPinnedUpstream` only looked for the YAML form (`UPSTREAM_SHA:`) and so never read the
  shell script's `UPSTREAM_SHA=`. A local build could have cloned a different upstream than CI
  with every gate green. `test_build_script_pins_the_same_sha` closes it.
- **`catalog/index.html` says where it comes from.** It is generated by
  `tools/gen_browser.py`, is committed by design, and carried no marker — so a hand edit looked
  legitimate and would be overwritten on the next regeneration. The generator now emits an
  `AUTO-GENERATED` header. `tests/run_all.py`'s staleness gate caught the change immediately,
  which is that gate working.
- **Removed `docs/packs/_packs_data.json`** (5,081 lines). Nothing read it, nothing generated
  it, and it still declared `catalog_version: 0.6.0` while the catalog had moved to 0.7.x — a
  stale second copy of the same counts this repository keeps taking trouble to have exactly one
  of. It stays recoverable from git history.

*Considered and deliberately not done:* renaming `tools/validate_catalog.py`, `tools/patch_ui.py`
and moving `catalog/ui-theme.json`. Each is named from roughly twenty places across the
CHANGELOG, both READMEs, both architecture documents and the test suite, so the rename would
touch far more than it would clarify. They are already described in the tool table in
`README.md`, which is where someone looking for them actually looks.
